...
四个，但是实际使用的只有用户态3和内核态0.

2. 不同特权级有什么区别？

特权指令只有在特权级下才能执行；

此外，在不同的特权级下，同一条指令对数据的访问权限也是不相同的，其行为可能也是不相同的。

3. 请说明CPL、DPL和RPL在中断响应、函数调用和指令执行时的作用。

发生中断响应或系统调用时，是从低优先级到高优先级，要判断当前特权级和待转移控制的优先级；

访问段的时候，是从高优先级访问低优先级，要求max(CPL,RPL)<=DPL。
...
指令在访问数据时，需要判断优先级是否足够，即max(CPL,RPL)<=DPL；

控制转移时，一般要求被移交代码的优先级高于当前优先级；

发生中断或异常时，需要判断当前门描述符的特权级和目标代码的特权级，要求门描述符拥有更高的特权。

2. 在什么情况下会出现特权级切换？

发生中断、异常、系统调用及处理此类事件结束时，会发生特权级切换。

当低特权级代码调用高特权级代码时，会引起特权级切换。

3. int指令在ring0和ring3的执行行为有什么不同？

在ring3中，int指令涉及特权级切换过程，需要先在系统堆栈中压入SS和ESP的内容。

4. 如何利用int和iret指令完成不同特权级的切换？

这需要人工改写系统堆栈的内容，设计一个专门的trap根据要求改写trap frame；当iret指令发生作用，CS寄存器被reload，这时只要手动弹出系统堆栈的ESP和SS并恢复到原本用户堆栈，即完成了用户态到内核态的切换。
...
最多两次，从虚拟地址，查询页表得到逻辑地址，再查询段表得到物理地址。

2. 描述X86-32的MMU地址转换过程；

根据虚拟地址查询页表，得到逻辑地址；再进一步查询段表得到实际的物理地址。
