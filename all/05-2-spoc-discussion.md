...
进程结束或进入等待状态；时间片用完；被高优先级进程抢占。

2. 分析ucore的进程切换代码，说明ucore的进程切换触发时机和进程切换的判断时机都有哪些。

schedule和proc_run可能触发进程切换，对应进程调度和运行时的切换。

switch_to实现进程的切换（保存和恢复上下文）。

3. ucore的进程控制块数据结构是如何组织的？主要字段分别表示什么？

PCB在ucore中是struct proc_struct
主要字段有mm_struct, wait_state, need_resche等属性，表示其对于的虚拟内存空间、进程状态、是否接受调度等等。

### 12.2 进程创建

1. fork()的返回值是唯一的吗？父进程和子进程的返回值是不同的。请找到相应的赋值代码。

是不唯一的。父进程返回子进程的pid，子进程返回0。
...
elf_header里有一个专门的属性，指定第一条指令开始的位置。

2. 第一个用户进程执行的代码在哪里？它是什么时候加载到内存中的？

它是放在ucore的内核代码之后的，和ucore一起被加载到内存中。
...
僵尸进程和孤儿进程代表父进程先退出了，导致子进程的资源一段时间内得不到回收。最后会由祖先进程进行回收。
